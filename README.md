...
El **HULK Compiler** es una implementación parcial de un compilador para el lenguaje de programación didáctico **HULK** (Havana University Language for Kompiers). El proyecto abarca un frontend funcional (análisis léxico, sintáctico, semántico e inferencia de tipos) y un backend en desarrollo con generación de código BANNER/VM además de un generador nativo limitado. El compilador está escrito en **C++17** y ofrece generación de ejecutables nativos a `./output` para un subconjunto de programas compatibles.
