...
InterpretResult VM::interpret(const BannerProgram& program) {
    // Ejecuta directamente el stream lineal de instrucciones BANNER de la
    // función de entrada (típicamente "_entry"). Soporta LOAD/STORE,
    // aritmética, control de flujo (LABEL/GOTO/IF_GOTO) e impresión, que es
    // lo que emite BannerGenerator para el código de nivel superior.
    using BVal = std::variant<double, std::string, bool, std::nullptr_t>;

    const BannerFunction* entry = nullptr;
    for (const auto& f : program.functions) {
        if (f.name == program.entryFunction) { entry = &f; break; }
    }
    if (!entry) return InterpretResult::INTERPRET_OK;

    std::unordered_map<std::string, std::string> dataMap;
    for (const auto& d : program.data) dataMap[d.label] = d.value;

    const auto& code = entry->instructions;
    std::unordered_map<std::string, size_t> labels;
    for (size_t i = 0; i < code.size(); ++i)
        if (code[i].kind == BannerInstr::Kind::LABEL)
            labels[code[i].label] = i;

    std::vector<BVal> st;
    std::unordered_map<std::string, BVal> vars;

    auto truthy = [](const BVal& v) -> bool {
        if (std::holds_alternative<std::nullptr_t>(v)) return false;
        if (std::holds_alternative<bool>(v)) return std::get<bool>(v);
        if (std::holds_alternative<double>(v)) return std::get<double>(v) != 0.0;
        return true;
    };
    auto num = [](const BVal& v) -> double {
        if (std::holds_alternative<double>(v)) return std::get<double>(v);
        if (std::holds_alternative<bool>(v)) return std::get<bool>(v) ? 1.0 : 0.0;
        return 0.0;
    };
    auto toStr = [](const BVal& v) -> std::string {
    ...
    auto pop = [&]() -> BVal { if (st.empty()) return nullptr; BVal v = st.back(); st.pop_back(); return v; };

    size_t ip = 0, steps = 0;
    const size_t kMaxSteps = 50000000;
    while (ip < code.size()) {
        if (++steps > kMaxSteps) break;
        const BannerInstr& in = code[ip];
        switch (in.kind) {
            case BannerInstr::Kind::LOAD:
                if (!in.varName.empty()) {
                    auto it = vars.find(in.varName);
                    st.push_back(it != vars.end() ? it->second : BVal(nullptr));
                } else if (!in.label.empty()) {
                    if (in.label == "true") st.push_back(true);
                    else if (in.label == "false") st.push_back(false);
                    else if (in.label == "nil") st.push_back(nullptr);
                    else { auto it = dataMap.find(in.label); st.push_back(it != dataMap.end() ? BVal(it->second) : BVal(in.label)); }
                } else {
                    st.push_back(in.number);
                }
                break;
            case BannerInstr::Kind::STORE: {
                BVal v = pop();
                if (in.varName != "_") vars[in.varName] = v;
                break;
            }
            case BannerInstr::Kind::ADD: {
                BVal b = pop(), a = pop();
                if (std::holds_alternative<std::string>(a) || std::holds_alternative<std::string>(b))
                    st.push_back(toStr(a) + toStr(b));
                else st.push_back(num(a) + num(b));
                break;
            }
            case BannerInstr::Kind::SUB: { BVal b = pop(), a = pop(); st.push_back(num(a) - num(b)); break; }
            case BannerInstr::Kind::MUL: { BVal b = pop(), a = pop(); st.push_back(num(a) * num(b)); break; }
            case BannerInstr::Kind::DIV: { BVal b = pop(), a = pop(); double d = num(b); st.push_back(d == 0.0 ? 0.0 : num(a) / d); break; }
            case BannerInstr::Kind::POW: { BVal b = pop(), a = pop(); st.push_back(std::pow(num(a), num(b))); break; }
            case BannerInstr::Kind::PRINT: { std::cout << toStr(pop()) << std::endl; break; }
            case BannerInstr::Kind::GOTO: {
                auto it = labels.find(in.label);
                if (it != labels.end()) { ip = it->second; continue; }
                break;
            }
            case BannerInstr::Kind::IF_GOTO: {
                BVal c = pop();
                if (!truthy(c)) { auto it = labels.find(in.label); if (it != labels.end()) { ip = it->second; continue; } }
                break;
            }
            case BannerInstr::Kind::CALL:
                if (in.varName == "print") { std::cout << toStr(pop()) << std::endl; st.push_back(nullptr); }
                else if (in.varName == "__not") { st.push_back(!truthy(pop())); }
                else if (in.varName == "__eq") { BVal b = pop(), a = pop(); st.push_back(toStr(a) == toStr(b)); }
                else { st.push_back(nullptr); }
                break;
            case BannerInstr::Kind::RETURN:
            case BannerInstr::Kind::HALT:
                return InterpretResult::INTERPRET_OK;
            default:
                break; // LABEL, PARAM, etc.
        }
        ip++;
    }
    return InterpretResult::INTERPRET_OK;
