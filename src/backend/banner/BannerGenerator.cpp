...
BannerProgram BannerGenerator::generate(const std::vector<std::unique_ptr<Stmt>>& statements) {
    ...
    // Crear función de entrada para ejecutar los statements top-level
    beginFunction("_entry", {});
...
int BannerGenerator::getLocalSlot(const std::string& name) {
    ...
    if (it != ctx.localSlots.end()) {
        // Local slots live after parameters in the frame
        return static_cast<int>(ctx.nextParamSlot) + it->second;
    }
...
void BannerGenerator::visitIfStmt(const IfStmt& stmt) {
    stmt.condition->accept(*this);
    stmt.thenBranch->accept(*this);
    if (stmt.elseBranch) stmt.elseBranch->accept(*this);
}

void BannerGenerator::visitWhileStmt(const WhileStmt& stmt) {
    stmt.condition->accept(*this);
    stmt.body->accept(*this);
}

void BannerGenerator::visitForStmt(const ForStmt& stmt) {
...
BannerGenerator::visitCallExpr(const CallExpr& expr) {
    ...
    if (expr.callee) {
        if (auto calleeVar = dynamic_cast<const VariableExpr*>(expr.callee.get())) {
            call.varName = calleeVar->name.lexeme;
        } else {
            call.varName = "__call";
        }
    }
